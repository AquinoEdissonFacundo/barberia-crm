/**
 * Starter Theme - Dashboard Configuration
 *
...
export const DASHBOARD_CONFIG = {
  ...
  sidebar: {
    ...
    navigation: {
       ...
       * These are added to the sidebar in addition to entity navigation
       */
      sections: [
        {
          id: 'analytics',
  ...
  // Allow the shop settings page even though it's not in the sidebar
  // (SettingsSidebar crashes for unknown page keys with missing icons)
  isSettingsPageEnabled(pageKey: string): boolean {
    if (pageKey === 'shop') return true
    const pages = this.settings.pages as Record<string, { enabled: boolean } | undefined>
...
export const isSettingsPageEnabled = (pageKey: string): boolean => {
  return DASHBOARD_CONFIG.isSettingsPageEnabled(pageKey)
}
