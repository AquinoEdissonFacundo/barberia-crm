...
// ============================================================
// BARBER CRM — OWNER LOGIN
// ============================================================

const BARBER_TEAM_ID = Cypress.env('BOOKING_TEAM_ID') || 'test-team'
const BARBER_OWNER_EMAIL = Cypress.env('OWNER_EMAIL') || 'aquinoedissonfacundo@gmail.com'
const BARBER_OWNER_PASSWORD = Cypress.env('TEST_PASSWORD') || 'Fadasa222'

/**
 * Login as BarberCRM Owner — the real team owner who has /dashboard access.
 * Uses credentials from CYPRESS_OWNER_EMAIL / CYPRESS_TEST_PASSWORD env vars.
 */
export function loginAsBarberOwner() {
  cy.session('barber-owner-session', () => {
    apiLogin(BARBER_OWNER_EMAIL, BARBER_OWNER_PASSWORD).then((apiLoginSucceeded) => {
      if (apiLoginSucceeded) {
        cy.visit('/dashboard', { timeout: 60000 })
      }
      cy.url().should('include', '/dashboard')
    ...
    validate: () => {
      ...
      cy.window().then((win) => {
        const teamId = win.localStorage.getItem('activeTeamId')
        expect(teamId, 'activeTeamId must be set').to.not.be.null
        expect(teamId, 'activeTeamId must not be empty').to.not.be.empty
      })
    },
  })
}
