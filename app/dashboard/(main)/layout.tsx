...
async function DefaultMainDashboardLayout({
  ...
  if (userId && teamId && pathname) {
    // Role-based routing: members (barbers) can only access their calendar and profile settings
    const roleRows = await queryWithRLS<{ role: string }>(
      `SELECT role FROM "team_members" WHERE "userId" = $1 AND "teamId" = $2 LIMIT 1`,
      [userId, teamId],
      userId
    )
    ...
    if (userRole === 'member' || userRole === 'viewer') {
      const allowed = ['/dashboard/calendar', '/dashboard/settings']
